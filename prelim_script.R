...
pairwiseCorrelations <- function(data_table, labels, outDir, filename, format = ".pdf", 
  ...
  if(format == ".pdf"){
    pdf(paste0(outDir, "/", filename, ".pdf"), height=4.5, width = 5.5)
  }
  ...
  for (i in 1:(ncol(data_table)-1)){
    for (j in (i+1):ncol(data_table)){
      ...
      if(p.val<=threshold){
        ...
        doc = officer::ph_with(x = doc, p, ph_location(type="body",width=6, height=4.5), res=600)
        }else{print(p)}
  ...
  if(format == ".pptx"){
   print(doc, target = paste0(outDir, "/", filename,".pptx")) 
  }else(dev.off())
